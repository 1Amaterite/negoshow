...
import {
  ComposedChart, LineChart, Area, Line, BarChart, Bar, XAxis, YAxis, ResponsiveContainer,
  Tooltip, CartesianGrid, ReferenceLine, Cell,
} from "@/components/Charts";

interface DashboardClientProps {
  initialCommodities: any;
  initialDescActivity: any;
  initialLastUpdate: any;
  initialPredData: any;
  initialDescPrices: any;
...
export default function DashboardClient({
  ...
  // Initialize state directly from the initialFirstId prop
  const [predCId, setPredCId] = useState<string | null>(initialFirstId);
  const timeframe = "7";

  const [descCId, setDescCId] = useState<string | null>(initialFirstId);
  ...
  const { data: dynamicCommodities = [], isLoading: isCommsLoading } = useQuery({
    ...
    initialData: initialCommodities,
  });
  ...
  const PredTip = ({ active, payload, label }: any) => {
    ...
    return (
      <div className="bg-background border border-border rounded-xl px-3 py-2 shadow-lg text-xs">
        <p className="font-bold text-foreground mb-1">{d?.araw || label}</p>
        {a?.value != null && <p className="text-primary font-semibold">{t.dashboard.actualPrice}: ₱{a.value}</p>}
        {h?.value != null && (
          <>
            <p className="text-amber-600 font-semibold">{t.dashboard.predictedPrice}: ₱{h.value}</p>
            {d.hulaMin != null && d.hulaMax != null && d.hulaMin !== d.hulaMax && (
               <p className="text-[10px] text-muted-foreground mt-1">Expected Range: ₱{d.hulaMin} - ₱{d.hulaMax}</p>
            )}
          </>
        )}
      </div>
  ...
  const processedPredData = predData.map((d: any) => ({
    ...d,
    hulaRange: d.hulaMin != null && d.hulaMax != null ? [d.hulaMin, d.hulaMax] : undefined
  }));
      ...
      <div className="dashboard-content px-5 md:px-10 lg:px-14 py-6 md:py-8">
        ...
        <section className="dashboard-section dashboard-forecast mt-8">
          ...
          <div className="bg-card rounded-xl border border-border overflow-hidden">
            ...
            {latestInsight && (
              <div className="flex items-start gap-2 px-4 py-2.5 bg-indigo-50 border-b border-indigo-100">
                <Info size={13} className="text-indigo-600 shrink-0 mt-0.5"/>
                <p className="text-xs font-medium text-indigo-900">
                  <span className="font-bold">AI Insight:</span> {latestInsight}
                </p>
              </div>
            )}
            <div className="px-2 pt-4 pb-3">
              ...
              {isPredLoading ? <div className="h-[280px] flex items-center justify-center text-xs text-muted-foreground">Loading chart...</div> : (
                <ResponsiveContainer width="100%" height={280}>
                  <ComposedChart data={processedPredData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(114,121,110,0.15)"/>
                    <XAxis dataKey="timestamp" type="number" scale="time" domain={['dataMin', 'dataMax']} tickFormatter={(unixTime) => new Date(unixTime).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} tick={{fontSize:9,fill:"#72796e"}} axisLine={false} tickLine={false}/>
                    ...
                    {todayPoint && <ReferenceLine x={todayPoint.timestamp} stroke="rgba(114,121,110,0.4)" strokeDasharray="4 4" label={{value:"Today",position:"top",fontSize:9,fill:"#72796e"}}/>}
                    <Area type="monotone" dataKey="hulaRange" stroke="none" fill="#fef3c7" fillOpacity={0.8} connectNulls={false} />
                    ...
                    <Line type="monotone" dataKey="hula"   name={t.dashboard.predictedPrice} stroke="#f59e0b" strokeWidth={2}   strokeDasharray="5 4" dot={(props: any) => props.payload.aktwal != null ? <circle key={props.key} /> : <circle cx={props.cx} cy={props.cy} r={3} fill="#f59e0b" stroke="none" key={props.key} />} connectNulls={false}/>
                  </ComposedChart>
                </ResponsiveContainer>
              )}
            </div>
