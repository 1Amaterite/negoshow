...
type DocKalagayan = "processing" | "validated" | "published" | "rejected";
...
const DOC_STATUS: Record<DocKalagayan, { label: string; cls: string }> = {
...
function KalagayanChip({ volatility }: { volatility: string }) {
  ...
  return (
    <div className="sticky top-0 z-20 bg-background/95 backdrop-blur-sm border-b border-border px-4 md:px-8 py-3 md:py-4">
      <div className="flex items-center gap-3">
        {onBack && (
          <button type="button" aria-label="Bumalik" onClick={onBack} className="w-9 h-9 rounded-full bg-muted flex items-center justify-center active:scale-95 transition-transform shrink-0">
...
function CommodityImage({ commodity, size = "md", className = "" }: { commodity: Commodity; size?: "sm" | "md" | "lg"; className?: string }) {
  const sizes = { sm: "w-8 h-8", md: "w-11 h-11", lg: "w-14 h-14" };
  return <img src={commodity.image} alt={commodity.tagalog} loading="lazy" className={`${sizes[size]} rounded-xl object-contain p-1 border border-border/70 bg-[#faf8f3] shrink-0 ${className}`} />;
}
    ...
    <div>
      ...
      <div className="px-4 pt-5">
        ...
        <div className="space-y-2 mb-5">
          {[
            { icon: BarChart2, label: "Dashboard ng Presyo", sub: "Mga kasalukuyang baseline presyo at analytics", screen: "dashboard" as Screen },
            { icon: FileText,  label: "Talaan ng mga Pinagmulan",      sub: "Mga DA/LGU bulletin na pinagkukunan",          screen: "transparency" as Screen },
      ...
      <div className="px-4 pt-5 space-y-4 pb-6">
        ...
        {!isFair && (
          <button onClick={onAdvisor} className="w-full bg-primary text-white font-bold text-sm py-4 rounded-full flex items-center justify-center gap-2 active:scale-[0.98] transition-all">
            <Navigation size={16}/>Tingnan ang Inirerekomendang Hakbang
          </button>
        )}
  ...
  return (
    <div className="dashboard-page">
      <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-sm border-b border-border px-5 md:px-10 lg:px-14 py-4 md:py-5">
        <h1 className="text-xl md:text-2xl font-extrabold text-foreground">Pangkalahatang Kalagayan ng Merkado</h1>
        <p className="text-sm text-muted-foreground mt-1">Kasalukuyang presyo, galaw, prediksyon, at mga rekomendasyon sa pagbili · Hulyo 10, 2026</p>
      </div>
      <div className="dashboard-content px-5 md:px-10 lg:px-14 py-6 md:py-8">
        ...
        <section className="dashboard-section dashboard-overview">
          <SL>Mahahalagang Sukatan</SL>
          <div className="grid grid-cols-2 gap-2 mb-5">
            <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
              <p className="text-xs text-muted-foreground mb-1">Katatagan ng Merkado</p>
            ...
            <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
              <p className="text-xs text-muted-foreground mb-1">Mga Pabago-bagong Kalakal</p>
            ...
            <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
              <p className="text-xs text-muted-foreground mb-1">Karaniwang Galaw ng Presyo</p>
          ...
          <div className="rounded-2xl border border-border overflow-hidden bg-card shadow-sm">
            <div className="grid grid-cols-[1fr_52px_64px_56px] bg-muted px-3 py-2 border-b border-border">
              {["Kalakal","Presyo","Galaw","Kalagayan"].map((h)=>(
        ...
        {/* DIAGNOSTIC */}
        <section className="dashboard-section dashboard-variance">
          <SL>Pagkakaiba ng Presyo — Kasalukuyan kumpara sa 30-Araw na Batayan</SL>
          <div className="bg-card rounded-2xl border border-border overflow-hidden shadow-sm mb-4">
            <div className="px-4 pt-4 pb-1">
              <div className="flex items-center gap-4 mb-3">
                ...
                <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm bg-[#c8a97a]"/><span className="text-[10px] text-muted-foreground font-semibold">30-Araw na Karaniwan</span></div>
              </div>
                ...
                <div key={d.name} className={`flex items-center justify-between rounded-xl px-4 py-3 border ${hi?"bg-red-50 border-red-200":"bg-green-50 border-green-200"}`}>
                  <div>
                    ...
                    <p className={`text-[10px] font-semibold ${hi?"text-red-600":"text-green-700"}`}>
                      {hi?`₱${d["Kasalukuyan"]-d["30-Araw na Karaniwan"]} na mas mataas kaysa 30-araw na karaniwan`:`₱${d["30-Araw na Karaniwan"]-d["Kasalukuyan"]} na mas mababa kaysa 30-araw na karaniwan`}
                    </p>
                  </div>
        ...
        {/* PREDICTIVE */}
        <section className="dashboard-section dashboard-forecast">
        ...
        <section className="dashboard-section dashboard-procurement-cta">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-primary mb-2">Sentro ng Pagbili</p>
              <h2 className="text-xl font-extrabold text-foreground">Gawing mas mahusay na desisyon sa pagbili ang datos ng merkado</h2>
              <p className="text-sm text-muted-foreground mt-1 max-w-2xl">Ihambing ang presyo ng mga supplier, suriin ang mga rekomendasyon, at alamin kung saan pinakamura ang bawat kalakal.</p>
            </div>
            <button onClick={onAdvisor} className="shrink-0 bg-primary text-white font-bold text-sm px-5 py-3 rounded-xl flex items-center justify-center gap-2 hover:opacity-90 transition-opacity">
              <Navigation size={15}/>Buksan ang Sentro ng Pagbili
            </button>
          </div>
        </section>
...
function ProcurementScreen({ commodities, onOpenAdvisor }: { commodities: Commodity[]; onOpenAdvisor: (c: Commodity)=>void }) {
...
function CommodityScreen({ commodity, onBack, onAdvisor }: { commodity: Commodity; onBack: ()=>void; onAdvisor: ()=>void }) {
  ...
  return (
    <div>
      <PageHeader title={commodity.tagalog} subtitle={commodity.name} onBack={onBack}/>
...
function AdminLoginScreen({ onLogin, onBack }: { onLogin: ()=>void; onBack: ()=>void }) {
  ...
  return (
    <div className="min-h-screen bg-muted/40">
      <PageHeader title="Pag-login ng Admin" onBack={onBack}/>
...
function AdminScreen({ onLogout, onBack }: { onLogout: ()=>void; onBack: ()=>void }) {
  ...
  return (
    <div className="admin-dashboard min-h-screen bg-muted/30">
      <PageHeader
        title="Dashboard ng Admin" subtitle="NegoShow Talipapa Utility" onBack={onBack}
        right={
...
function DesktopNav({ activeTab, navigate }: { activeTab: string; navigate: (s: Screen)=>void }) {
  const links = [
    { id:"home", label:"Tahanan", screen:"home" as Screen },
    { id:"checker", label:"Suriin ang Presyo", screen:"checker" as Screen },
    { id:"trends", label:"Pagsusuri ng Merkado", screen:"dashboard" as Screen },
    { id:"procurement", label:"Pagbili", screen:"procurement" as Screen },
    { id:"more", label:"Mga Sanggunian", screen:"more" as Screen },
  ];
  return (
    <header className="hidden md:block sticky top-0 z-50 bg-background/95 backdrop-blur-md border-b border-border">
      <div className="w-full px-8 lg:px-12 h-20 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-1" aria-label="Main navigation">
          {links.map((link)=><button key={link.id} onClick={()=>navigate(link.screen)} className={`px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors ${activeTab===link.id?"bg-primary text-white":"text-muted-foreground hover:bg-muted hover:text-foreground"}`}>{link.label}</button>)}
        </nav>
...
function BottomNav({ activeTab, navigate }: { activeTab: string; navigate: (s: Screen)=>void }) {
  const tabs = [
    ...
    { id:"trends",  label:"Pagsusuri",icon:TrendingUp,    screen:"dashboard" as Screen },
    { id:"procurement", label:"Bumili", icon:Navigation, screen:"procurement" as Screen },
...
export default function App() {
  ...
  const activeTab =
    ...
    : ["dashboard","commodity"].includes(screen) ? "trends"
    : ["procurement","advisor"].includes(screen) ? "procurement"
    : "more";
    ...
    <div className="app-root bg-background" style={{ fontFamily:"'Be Vietnam Pro', sans-serif" }}>
      <div className="app-shell bg-background">
        ...
        <main id="main-content" className={`app-main ${isAdminFlow ? "" : "pb-20 md:pb-0"}`}>
          ...
          {screen==="dashboard"   && <DashboardScreen commodities={COMMODITIES} onSelectCommodity={(c)=>navigate("commodity",c)} onAdvisor={()=>navigate("procurement")}/>}
          {screen==="procurement" && <ProcurementScreen commodities={COMMODITIES} onOpenAdvisor={(c)=>{ setCheckerCommodity(c); setQuotedPrice(""); navigate("advisor"); }}/>}
          {screen==="commodity"   && selectedCommodity && <CommodityScreen commodity={selectedCommodity} onBack={()=>navigate("dashboard")} onAdvisor={()=>navigate("procurement")}/>}
